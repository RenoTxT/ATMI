...
const int jumlahMahasiswa = 8, jumlahProdi = 6, jumlahJenjang = 2;
...
void inputdata() {
    ...
    for (int i = 0; i < jumlahMahasiswa; i++) {
        ...
        // Assign program and level based on NIM range
        if (nim[i] >= "20221001" && nim[i] <= "20221099") {
            ...
            i--; // Ulangi input untuk NIM yang tidak valid
        }
    }
...
void sortdatamahasiswa() {
    ...
    for (int i = 0; i < jumlahMahasiswa; i++) {
        ...
        // Kembali assign program dan tingkatan berdasarkan NIM
        if (nim[i] >= "20221001" && nim[i] <= "20221099") {
            prodiMahasiswa[i] = prodi[0]; // TMI
            jenjangMahasiswa[i] = jenjang[0];
        }
